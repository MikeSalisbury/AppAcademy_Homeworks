...
class MetaCorgiSnacks
  ...
  def method_missing(name, *args)
    # Your code goes here...
    method_info = @snack_box.send("get_#{name}_info", @box_id)
    method_tastiness = @snack_box.send("get_#{name}_tastiness", @box_id)
    method_name = "#{name.to_s.split('_').map(&:capitalize).join(' ')}"
    result = "#{method_name}: #{method_info}: #{method_tastiness} "
    if method_tastiness > 30
      return "* #{result}"
    else
      result
    end
  ...
  def self.define_snack(name)
    ...
    define_method(name) do
      ...
      result = "#{display_name}: #{method_info}: #{method_tastiness}"
      if method_tastiness > 30
         return "* #{result}"
      else
        result
